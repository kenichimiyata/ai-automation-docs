...
**GitHub Issue を書くだけで、AI が自動的に実装・応答するシステムです。**

| 操作 | 自動実行されること |
|------|--------------------|
| GitHub Issue を作成する | GitHub Actions が起動 |
| | Supabase（DB）に Issue が自動保存される |
| | VS Code Copilot Chat に自動入力される |
| | AI の応答が Supabase に記録される |

> **開発者がやること：** Issue を書くだけ。  
> **AI がやること：** 分析・実装・記録。

詳しい技術構成は [Architecture](#architecture) セクションへ。
